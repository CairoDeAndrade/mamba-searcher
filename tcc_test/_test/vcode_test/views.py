...
def mamba(request, ):
        ...
        try:
            ...
            for palavra in novas_palavras:
                if palavra in novo_texto:
                    sum += 1
                    contained_words.append(palavra)
                else:
                    continue
    ...
    for files_name, word_qtd, word in zip(list_files_name, list_word_qtd, contained_words):
        complete_list = [files_name, word_qtd, word]
        real_final.append(complete_list)

    return render(request, 'vcode_test/mamba.html', {'novas_palavras': novas_palavras,
